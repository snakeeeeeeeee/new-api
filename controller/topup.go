...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		completed, err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.TradeNo)
		if err != nil {
			log.Printf("易支付回调处理充值订单失败: %v, verifyInfo=%v", err, verifyInfo)
			return
		}
		if completed.Updated {
			log.Printf("易支付回调更新用户成功 %v", completed.TopUp)
			notifyExternalTopUpSuccessAsync(completed)
		}
	} else {
