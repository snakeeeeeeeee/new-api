...
const schemaDescriptions = {
  ...
  VideoTaskInput:
    'Prompt plus provider-neutral URL-only reference images, videos, and audios. Non-generation operations require a separate input.video source. leonardo-seedance-* accepts media references; leonardo-minimax-h3-1440p accepts frame or image references only.',
...
const schemaPropertyOverrides = {
  ...
  VideoTaskInput: {
    ...
    reference_mode:
      'frame accepts ordered start/end images; images accepts ordinary image references; media accepts grouped image/video/audio references where supported. leonardo-seedance-* uses media with 4 images and 3 videos, while leonardo-minimax-h3-1440p uses frame with at most 2 images or images with at most 5 images.',
    reference_videos:
      'Up to three URL-only video references in media mode where supported; MiniMax H3 rejects video references.',
    reference_audios:
      'Up to three URL-only audio references in media mode for models that support audio references; leonardo-* models reject this field.',
