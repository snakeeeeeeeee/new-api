...
const leonardoVideoModelCapabilities = {
  ...Object.fromEntries(
    [
  ...
  'leonardo-minimax-h3-1440p': {
    ...
    reference_modes: {
      ...
      media: {
        ...
        minimum_audios: 1,
        maximum_audios: 3,
        maximum_total_audio_duration_seconds: 15,
      },
    },
...
const videoTaskCreateExamples = {
  ...
  leonardoMedia: {
    summary: 'Leonardo Seedance with image, video, and audio references',
    description:
      'Leonardo requires media when references are present and accepts up to four images, three videos, and one audio reference. Audio requires at least one image or video; its duration is detected by the service.',
    value: {
      ...
      input: {
        ...
        reference_audios: [
          { url: 'https://media.example.com/leonardo/soundtrack.wav' },
        ],
      },
      output: {
        duration: 4,
  ...
  leonardoMiniMaxH3Frame: {
    summary: 'Leonardo MiniMax H3 start and end frame generation',
    description:
      'MiniMax H3 is fixed at 1440p, accepts 5-15 seconds, and uses frame for one start image plus an optional end image.',
    value: {
      model: 'leonardo-minimax-h3-1440p',
      operation: 'generation',
      input: {
        prompt:
          'Move smoothly from sunrise to a clear night sky while preserving the landscape',
        reference_mode: 'frame',
        image: { url: 'https://media.example.com/h3/start.png' },
        reference_images: [{ url: 'https://media.example.com/h3/end.png' }],
      },
      output: {
        duration: 5,
        aspect_ratio: '16:9',
  ...
  leonardoMiniMaxH3Media: {
    summary: 'Leonardo MiniMax H3 image and audio references',
    description:
      'MiniMax H3 media mode accepts one to five images plus one to three audio references, rejects video references, and always generates native output audio.',
    value: {
      model: 'leonardo-minimax-h3-1440p',
      operation: 'generation',
      input: {
        prompt: 'Follow the musical rhythm while preserving the subject',
        reference_mode: 'media',
        image: { url: 'https://media.example.com/h3/subject.png' },
        reference_audios: [{ url: 'https://media.example.com/h3/rhythm.wav' }],
      },
      output: {
        duration: 5,
        aspect_ratio: '16:9',
  ...
  paths: {
    ...
    '/v1/video/tasks': {
      post: {
        ...
        description:
          'Creates a normalized generation, edit, extension, or remix task. Provider capability and parameter limits are validated before precharge and upstream submission. output.generate_audio is the provider-neutral generated-audio control and defaults to true on supported providers; provider_options.*.generate_audio and provider_options.*.reference_mode are rejected. Adobe video prompts accept at most 1200 Unicode characters. Adobe Kling/Veo model SKUs fix the output resolution and reject output.resolution. Kling 3.0 uses 3-15 second frame generation and requires at least one frame image. Kling 3.0 Omni additionally accepts up to three images. Veo 3.1 accepts 4, 6, or 8 seconds; Standard images mode requires 8 seconds and 16:9, while Fast is frame-only. Leonardo-prefixed models accept HTTP(S) reference URLs and reject data URLs, base64, and provider files. The gateway applies only common reference validation and a 30-image, 10-video, 10-audio, 50-total safety envelope to Leonardo requests; the downstream Leonardo service validates concrete model reference counts, combinations, formats, and media durations. Exact model mapping, output duration, aspect ratio, and MiniMax H3 native-audio requirements are still validated before billing. Compatibility endpoints under /v1/videos remain unchanged.',
        'x-description-zh-CN':
          '创建规范化的视频生成、编辑、扩展或 Remix 任务，并在预扣费及上游提交前完成模型能力校验。output.generate_audio 是供应商无关的生成音轨开关，支持该能力的渠道省略时默认 true；provider_options.*.generate_audio 和 provider_options.*.reference_mode 会被拒绝。Adobe 视频提示词最多 1200 个 Unicode 字符。Adobe Kling/Veo 模型名固定输出分辨率并拒绝 output.resolution。Kling 3.0 使用 3-15 秒 frame 模式且至少需要一张帧图；Kling 3.0 Omni 额外支持最多三图。Veo 3.1 仅支持 4、6、8 秒；Standard 的 images 模式固定为 8 秒、16:9，Fast 仅支持 frame。leonardo-* 模型接受 HTTP(S) 参考 URL，拒绝 Data URL、Base64 和 provider file。网关对 Leonardo 请求只执行公共参考素材校验及 30 图、10 视频、10 音频、总计 50 项的安全上限；具体模型的素材数量、组合、格式和时长由下游 Leonardo 服务校验。精确模型映射、输出时长、画面比例和 MiniMax H3 原生音轨要求仍在计费前校验。原有 /v1/videos 兼容接口保持不变。',
      ...
      VideoTaskInput: {
        ...
        properties: {
          ...
          reference_mode: {
            ...
            default: 'frame',
            description:
              'frame accepts image references only and maps at most two images to start/end frames. images accepts up to 5 image references. media has a gateway safety envelope of 30 images, 10 videos, 10 audio files, and 50 references total; provider adaptors validate concrete model limits.',
          },
          image: {
            ...ref('VideoTaskSource'),
            description:
          ...
          reference_videos: {
            ...
            items: ref('VideoTaskSource'),
            description:
              'Reference videos for media mode. Each provider validates concrete format, per-file duration, and total duration limits.',
          },
          reference_audios: {
            type: 'array',
            minItems: 1,
            ...
            items: ref('VideoTaskSource'),
            description:
              'Reference audio files for media mode. Each provider validates concrete format, per-file duration, and total duration limits.',
          },
          video: {
...
const schemaPropertyOverrides = {
  ...
  VideoTaskInput: {
    ...
    reference_audios:
      'URL-only audio references in media mode where supported. Leonardo Seedance accepts at most 1 audio with an image or video; MiniMax H3 accepts 1-3 audios with 1-5 images. The caller does not provide duration; the downstream service checks the real file.',
  ...
  VideoTaskOutput: {
    ...
    resolution:
      'Optional only for models that expose selectable resolution. Omit this field for leonardo-minimax-h3-1440p because the model is fixed at 1440p.',
  },
  VideoTaskResultVideo: {
