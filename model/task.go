...
func InitTask(platform constant.TaskPlatform, relayInfo *commonRelay.RelayInfo) *Task {
	...
	if relayInfo != nil && relayInfo.ChannelMeta != nil {
		if relayInfo.ChannelMeta.ChannelType == constant.ChannelTypeGemini ||
			...
			(relayInfo.ChannelMeta.ChannelType == constant.ChannelTypeXai && relayInfo.ChannelMeta.ChannelOtherSettings.IsXAI2KEN()) {
...
func TaskGetAllUserTask(userId int, startIdx int, num int, queryParams SyncTaskQueryParams) []*Task {
	...
	// 获取数据。channel_id 需要保留给 DTO 层推导展示平台；普通用户响应会在 controller 层清空该字段。
...
// PersistTaskSubmitResult merges the executor's submit response into the
...
func PersistTaskSubmitResult(taskID int64, upstreamTaskID string, taskData json.RawMessage, consumeLogId int) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if consumeLogId > 0 && privateData.BillingContext != nil {
			privateData.BillingContext.ConsumeLogId = consumeLogId
		}
		updates := map[string]any{
			"private_data": privateData,
			"updated_at":   time.Now().Unix(),
		}
		if current.Status != TaskStatusSuccess && current.Status != TaskStatusFailure && len(taskData) > 0 {
