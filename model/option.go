...
func UpdateOption(key string, value string) error {
	...
	if key == "VideoPricing" {
		if err := ratio_setting.ValidateVideoPricingJSON(value); err != nil {
			return err
		}
	}
...
func updateOptionMap(key string, value string) (err error) {
	...
	if handled, handleErr := handleConfigUpdate(key, value); handleErr != nil {
		return handleErr
	} else if handled {
		return nil // 已由配置系统处理
	}
	...
	case "TokenTierPricingRules":
		err = ratio_setting.UpdateTokenTierPricingRulesByJSONString(value)
	case "ImageRatio":
...
func normalizeImageHandleOptionValue(configKey, value string) string {
	...
	case "sync_image_result_policy":
		return image_handle_setting.NormalizeSyncImageResultPolicy(value)
	case "sync_image_default_format":
		return image_handle_setting.NormalizeSyncImageDefaultFormat(value)
	case "debug_upstream", "sync_image_enabled":
...
func validateViolationConfigUpdate(configKey, value string) error {
	...
	default:
		return errors.New("unknown violation setting key")
	}
