...
const (
	...
	aggregateRouteRPMRedisPrefix   = "new-api:aggregate_route_rpm:v1"
...
func buildAggregateRouteRPMBaseKey(groupName string, modelName string, routeGroup string) string {
	return common.Sha1([]byte(groupName + "\n" + modelName + "\n" + routeGroup))
}

func buildAggregateRouteRPMKey(groupName string, modelName string, routeGroup string, metric string, unixSecond int64) string {
	return fmt.Sprintf("%s:%s:%s:%d", aggregateRouteRPMRedisPrefix, buildAggregateRouteRPMBaseKey(groupName, modelName, routeGroup), metric, unixSecond)
}
...
func recordAggregateRouteRPMFromContext(c *gin.Context, modelName string, routeGroup string, metric string) {
	...
	recordAggregateRouteRPM(aggregateGroup, modelName, routeGroup, metric)
}
...
func recordAggregateRouteRPM(groupName string, modelName string, routeGroup string, metric string) {
	...
	second := now.Unix()
	key := buildAggregateRouteRPMKey(groupName, modelName, routeGroup, metric, second)
...
func GetAggregateRouteRPMStats(groupName string, modelName string, routeGroup string) AggregateRouteRPMStats {
	return AggregateRouteRPMStats{
		RPM:        int(sumAggregateRouteRPM(groupName, modelName, routeGroup, aggregateRouteRPMMetricAttempt)),
		SuccessRPM: int(sumAggregateRouteRPM(groupName, modelName, routeGroup, aggregateRouteRPMMetricSuccess)),
		FailureRPM: int(sumAggregateRouteRPM(groupName, modelName, routeGroup, aggregateRouteRPMMetricFailure)),
	}
}

func sumAggregateRouteRPM(groupName string, modelName string, routeGroup string, metric string) int64 {
	if groupName == "" || modelName == "" || routeGroup == "" || metric == "" {
		return 0
	}
	...
	if common.RedisEnabled && common.RDB != nil {
		total, ok := sumAggregateRouteRPMRedis(groupName, modelName, routeGroup, metric, from, now)
		if ok {
			return total
		}
	}
	return sumAggregateRouteRPMMemory(groupName, modelName, routeGroup, metric, from, now)
}

func sumAggregateRouteRPMRedis(groupName string, modelName string, routeGroup string, metric string, from int64, to int64) (int64, bool) {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	...
	for second := from; second <= to; second++ {
		cmds = append(cmds, pipe.Get(ctx, buildAggregateRouteRPMKey(groupName, modelName, routeGroup, metric, second)))
	}
...
func sumAggregateRouteRPMMemory(groupName string, modelName string, routeGroup string, metric string, from int64, to int64) int64 {
	aggregateRouteRPMMemoryMu.Lock()
	defer aggregateRouteRPMMemoryMu.Unlock()
