...
## Requirements
- 用户管理邀请信息拆成余额消费、订阅使用、总使用；列表首屏不被日志聚合阻塞。
- 邀请统计页在现有余额消费外，增加订阅额度使用和订阅包购买金额。
- 订阅购买金额按成功订单 `subscription_orders.money` 统计，使用 `complete_time` 过滤。

## Technical Decisions
| Decision | Rationale |
|----------|-----------|
| 用户管理用异步批量接口补齐当前页拆分 | 累计日志扫描可能较慢，避免影响翻页/搜索首屏。 |
| 总使用仍用 `users.used_quota` | 它是最稳定累计值，不依赖日志完整性。 |
| 历史未标记日志归入余额/未标记消费 | 旧日志没有 `billing_source`，不能判为订阅。 |
| 订阅额度使用和订阅购买分开展示 | 额度消耗不是收入，真实收入来自成功订单。 |
